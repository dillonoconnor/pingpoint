defmodule PingpointWeb.CoreComponents do
  @moduledoc """
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        ...
        <p :if={@subtitle != []} class="mt-2 text-md leading-6">
  ...
  def table(assigns) do
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        ...
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group bg-zinc-50">
            <td
            ...
            >
              <div class="block py-4 pr-6">
                <span class="absolute -inset-y-px right-0 -left-4 bg-slate-300 group-hover:bg-slate-200 sm:rounded-l-xl" />
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                <span class="absolute -inset-y-px -right-4 left-0 bg-slate-300 group-hover:bg-slate-200 sm:rounded-r-xl" />
                <span
                  :for={action <- @action}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PingpointWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PingpointWeb.Gettext, "errors", msg, opts)
    end
  end
